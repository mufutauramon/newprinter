...
export default async function (context, req) {
  try {
    ...
    const plan = String(b.plan || "").trim();   // NEW

    // 1) validation
    if (!email || !password || !fullName || !phone || !plan) {
      return json(context, 400, { error: "fullName, phone, email, password, and plan are required" });
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
